...
public class BMI extends JFrame implements ActionListener {
	...
	private boolean isViableWeight(JTextField f) {
		...
		try {
			...
			if ((20 < p.getWeight()) && (p.getWeight() < 300)) {
				...
				msg1.setText("Ihre Gewichtsangabe liegt außerhalb des unterstützten Bereiches.");
				return false;
			}
	...
	private boolean isViableHeight(JTextField f) {
		...
		try {
			...
			if ((p.getHeight() > 0) && (p.getHeight() < 2.4)) {
				...
				msg2.setText("Ihre Größenangabe liegt außerhalb des unterstützten Bereiches");
				return false;
			}
		} catch (NumberFormatException ex) {
	...
	public void actionPerformed(ActionEvent e) {
		if (isViableWeight(gewicht) && isViableHeight(groesse)) {
			ergebnis.setText("Der BMI von " + p.getnName() + ", " + p.getvName() + " beträgt: " + p.berechnen(0));
			String[] bed = { "Sie sind untergewichtig.", "Sie haben ein normales Gewicht",
					"Sie sind leicht übergewichtig.", "Sie sind übergewichtig", "Sie sind stark übergewichtig." };
